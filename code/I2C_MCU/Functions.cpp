...
bool loadFromTeensy(){
  int datas[256]; //buffer that stores the requested datas from the teensy
  int v = 0; //index, useful to store the requested datas
  int j = 0;
  ...
  for(int i = 0; i<8;i++){ //covers the 8 datas 32bytes chunks
    Wire.beginTransmission(TEENSY_ADDR);  
    Wire.write(i+1);                   
    Wire.endTransmission(); 
    delay(50); 
